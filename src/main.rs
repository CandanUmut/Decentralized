...
use routes::{root, add_file, fetch_content, pin_file, add_folder, unpin_file, add_zip,  receive_messages, connect_peer, get_peers};
...
impl MyBehaviour {
    fn new(local_peer_id: PeerId) -> Result<Self> {
        ...
        Ok(Self { kademlia, mdns, messaging, relay, autonat })
    }
...
fn parse_peer_id(peer_id: &str) -> Result<libp2p::PeerId, String> {
    ...
    if let Ok(decoded_peer_id) = bs58::decode(peer_id).into_vec() {
        libp2p::PeerId::from_bytes(&decoded_peer_id)
            .map_err(|e| format!("Failed to convert decoded peer_id: {:?}", e))
    } else {
        Err("Invalid Base58 encoding for peer_id".to_string())
    }
}
...
async fn turn_binding() -> Result<(String, u16), String> {
    // TURN server details with UDP protocol only
    let turn_servers = vec![
        ("172.29.82.180", 3478, "udp"),
        ("172.29.82.180", 5349, "udp"), // Ensure this port supports UDP if used
    ];

    // Credentials - Must match the server's configuration
    let username = "myusername"; // Replace with your username
    let password = "b414420fc62b8c3e6f133cdfe77e6cd7"; // Replace with your password
    let realm = "172.29.82.180"; // Must match the server's realm

    let mut last_error = String::new();
    

    for (host, port, protocol) in turn_servers {
        ...
        // Focus only on UDP protocol
        if protocol.to_lowercase() != "udp" {
            println!("Skipping unsupported protocol: {}", protocol);
            continue;
        }

        // Attempt to bind a UDP socket
        ...
        // Create TURN client configuration for UDP
        ...
        // // Debugging TURN client configuration
        // println!("TURN Client Config (UDP):");
        // println!("  STUN Server Address: {}", client_config.stun_serv_addr);
        // println!("  TURN Server Address: {}", client_config.turn_serv_addr);
        // println!("  Username: {}", client_config.username);
        // println!("  Password: {}", client_config.password);
        // println!("  Realm: {}", client_config.realm);
...
async fn main() -> Result<()> {
        ...
        loop {
            ...
            match async {
                while let Some(event) = ipfs_event_rx.recv().await {
                    ...
                    match event {
                        IPFSEvent::AddFile { path } => {
                            println!("🛠️ Processing AddFile event for path: {}", path);
                            match ipfs_handler_clone.lock().await.add_file(&path, None).await {
                                Ok(cid) => {
                                    println!("✅ File added to IPFS with CID: {}", cid);
                                }
                                Err(e) => {
                        ...
                        IPFSEvent::DiscoverContent { cid } => {
                            ...
                            if let Err(e) = ipfs_handler_clone.lock().await.discover_content(&cid).await {
                    ...
                    if last_refresh.elapsed() > Duration::from_secs(300) { // Refresh every 5 minutes
                        println!("🔄 Refreshing IPFS node...");
                        match ipfs_handler_clone.lock().await.refresh().await {
                            Ok(_) => println!("✅ IPFS node refreshed successfully."),
                            Err(e) => eprintln!("❌ Failed to refresh IPFS node: {:?}", e),
                        }
                        last_refresh = Instant::now(); // Update the last refresh time
                    }
                }
                Ok::<(), ()>(())
            }
    ...
    SwarmBuilder::with_existing_identity(local_key.clone())
        ...
        .with_quic()
        .with_behaviour(|key| {
            MyBehaviour::new(key.public().to_peer_id()).expect("Failed to create behaviour")
        })?
...
println!("Swarm initialized with all capabilities!");
    ...
    tokio::spawn(async move {
        let app = Router::new()
            ...
            // .route("/connect_peer", post(connect_peer)) // Connect to a specific peer
            .layer(axum::Extension(ipfs_handler_clone)) // Share the IPFSHandler
    ...
    {
        let mut ipfs_handler = ipfs_handler.lock().await;
            ...
            event = async {
                ...
                match event {
                                    ...
                                    for peer_info in ok.peers {
                                        ...
                                        if let Ok(libp2p_peer_id) = libp2p::PeerId::from_bytes(&discovered_peer_id.to_bytes()) {
                                            ...
                                            if !manager.is_registered(&ipfs_peer_id) {
                                                manager.add_mapping(ipfs_peer_id.clone(), wallet_address.clone());
                                                println!(
                                                    "DHT Peer registered: {} with Wallet: {}",
                                                    ipfs_peer_id, wallet_address
                                                );
                                            }
                                        }
                                    ...
                                    if let Ok(libp2p_peer_id) = libp2p::PeerId::from_bytes(&peer_id.to_bytes()) {
                                        ...
                                        // Lock the PeerManager and add the mapping
                                        {
                                            ...
                                            if !manager.is_registered(&ipfs_peer_id) {
                                                manager.add_mapping(ipfs_peer_id.clone(), wallet_address.clone());
                                                info!(
                                                    "mDNS Peer registered: {} with Wallet: {}",
                                                    ipfs_peer_id, wallet_address
                                                );
                                            }
                                        }
                            ...
                            RequestResponseEvent::Message { peer, message } => match message {
                                RequestResponseMessage::Request { request, channel, .. } => {
                                    ...
                                    let response = b"Message received!".to_vec();
                                    {
                                        ...
                                        if let Err(err) = swarm.behaviour_mut().messaging.send_response(channel, response) {
                    ...
                    SwarmEvent::OutgoingConnectionError { peer_id, error, .. } => {
                        info!(
                            "Failed to establish connection to peer: {:?}. Error: {:?}",
                            peer_id, error
                        );
                    }
                    _ => {
