...
use anyhow::Result; // Import anyhow's Result for simplified error handling
use ipfs::{
...
use tokio::{fs, task, time::{sleep, Duration}};
...
impl IPFSHandler {
    ...
    pub async fn initialize_ipfs_daemon() -> Result<()> {
        ...
        if is_running {
            println!("🔍 Found an existing IPFS daemon instance. Terminating it...");
            
            // Kill the running IPFS daemon
            ...
            match kill_status {
                ...
                Ok(output) => {
                    eprintln!("❌ Failed to terminate the IPFS daemon. Error: {:?}", String::from_utf8_lossy(&output.stderr));
                    return Err(anyhow::anyhow!("Failed to stop existing IPFS daemon."));
                }
                Err(e) => {
                    eprintln!("❌ Error while trying to stop IPFS daemon: {:?}", e);
                    return Err(anyhow::anyhow!("Error stopping IPFS daemon."));
                }
            }
        } else {
            println!("ℹ️ No running IPFS daemon found. Starting fresh...");
        }
    
        // Start a new IPFS daemon instance
        println!("🚀 Starting a new IPFS daemon instance...");
        let status = tokio::process::Command::new("ipfs")
            .arg("daemon")
            .spawn();
    
        match status {
            Ok(mut child) => {
                // Wait for a few seconds to allow the daemon to initialize
                tokio::time::sleep(std::time::Duration::from_secs(5)).await;
    
                if child.try_wait()?.is_some() {
                    println!("✅ IPFS daemon started successfully.");
                } else {
                    eprintln!("❌ Failed to confirm IPFS daemon startup. It might still be initializing.");
                }
            }
            Err(e) => {
                eprintln!("❌ Failed to start the IPFS daemon. Error: {:?}", e);
                return Err(anyhow::anyhow!("Failed to start IPFS daemon."));
            }
        }
    ...
    pub async fn add_file(&self, path: &str, content_base64: Option<&str>) -> Result<String> {
        // Read file content based on the provided `path` or `content_base64`
        ...
        // Debug log to verify the content
        println!(
        ...
        // Compute CID
        let multihash = MultihashCode::Sha2_256.digest(&file_content);
        let cid = Cid::new_v1(CidCodec::Raw, multihash); // Use `Raw` for file-like data
    
        // Debug log to verify the CID
        println!("Computed CID: {}", cid);
    
        // Create and add block to IPFS
        let block = Block::new(file_content.into_boxed_slice(), cid.clone());
        self.ipfs.put_block(block).await.map_err(|e| {
            anyhow::anyhow!(
                "Failed to store block in IPFS. CID: {}. Error: {}",
                cid,
                e
            )
        })?;
    
        // Announce CID to the network
        self.ipfs.provide(cid.clone()).await.map_err(|e| {
            anyhow::anyhow!(
                "Failed to announce CID to the network. CID: {}. Error: {}",
                cid,
                e
            )
        })?;
    ...
    pub async fn add_folder(&self, folder_path: &str) -> Result<String> {
        ...
        let mut folder_cid_map = Vec::new(); // Store CIDs for each file
        ...
        while let Some(entry) = dir_entries.next_entry().await? {
            ...
            if file_type.is_file() {
                ...
                if let Some(file_path) = path.to_str() {
                    ...
                    let cid = self.add_file(file_path, None).await?;
                    folder_cid_map.push((file_path.to_string(), cid));
                }
            } else if file_type.is_dir() {
                // Box the recursive call to handle the async recursion
                if let Some(dir_path) = path.to_str() {
                    println!("Recursively adding folder: {}", dir_path);
                    let folder_cid = Box::pin(self.add_folder(dir_path)).await?;
