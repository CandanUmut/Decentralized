...
pub async fn root() -> Html<&'static str> {
    Html("<h1>Welcome to the Decentralized Network!</h1>")
}
...
pub async fn add_file(
    ...
    // Validate file type
    // let allowed_extensions = vec![".pdf", ".tar", ".tar.gz", ".txt", ".jpg", ".png", ".html", ".css", ".js", ".xlsx"];
    // if !allowed_extensions.iter().any(|ext| path.ends_with(ext)) {
    //     return Json(json!({"error": "Unsupported file type"}));
    // }
    ...
    match result {
        Ok(cid) => {
            ...
            match pin_result {
                Ok(_) => Json(json!({"cid": cid, "status": "File added and pinned"})),
                Err(e) => Json(json!({"cid": cid, "warning": format!("File added but failed to pin: {}", e)})),
            }
        }
        Err(e) => {
